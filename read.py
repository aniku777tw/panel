...
data_name = 'g_1'
data_path = r"D:\Solar panels\20211026\\" +data_name +"_RT"
...
for i,f in enumerate(files) :
    ...
    for cf in current_file:
        average_hsi_sample.append(cf[0:200])
        f = f.split('\\')[-1]
        if 'b' in  f:
            gt.append(0)
        elif 'w' in  f:
...
for i in range(len(gt)):
    if  gt[i]==0:
        cb,=plt.plot(range(200),snv_ahs[i],'b') 
    elif gt[i]==1:
        cw,=plt.plot(range(200),snv_ahs[i],'r') 
    elif gt[i]==2:
        l,=plt.plot(range(200),snv_ahs[i],'g') 
    elif gt[i]==3:
        g,=plt.plot(range(200),snv_ahs[i],'y') 
...
# train_smaple,test_smaple,train_gt,test_gt = train_test_split(pca_res,gt,test_size=0.5,random_state=8787)
# svm = SVC()
